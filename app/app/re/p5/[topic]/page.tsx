...
export async function generateStaticParams() {
  return P5_RE_TOPICS.map((topic) => ({ topic: topic.id }));
}
...
export default async function P5ReTopicPage({ params }: { params: Promise<{ topic: string }> }) {
  ...
  return (
    <>
      <Link href="/re/p5" className="back">← P5 Religious Education</Link>
      ...
      <section id="quick-quiz" className="quiz-panel">
        ...
        <Quiz questions={topic.quiz} topicId={topic.id} topicTitle={topic.title} backHref="/re/p5" />
      </section>
    </>
  );
}
