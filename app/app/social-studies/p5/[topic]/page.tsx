...
export async function generateStaticParams() {
  return P5_SOCIAL_TOPICS.map((topic) => ({ topic: topic.id }));
}

export default async function P5SocialStudiesTopicPage({ params }: { params: Promise<{ topic: string }> }) {
  ...
  return (
    <>
      <Link href="/social-studies/p5" className="back">← P5 Social Studies</Link>
      ...
      <section id="quick-quiz" className="quiz-panel">
        ...
        <Quiz questions={topic.quiz} topicId={topic.id} topicTitle={topic.title} backHref="/social-studies/p5" />
      </section>
    </>
  );
}
