...
export function generateStaticParams() {
  return P6_SOCIAL_TOPICS.map((t) => ({ topic: t.id }));
}

export default async function P6SocialPracticePage({
  params,
}: {
  ...
  return (
    <>
      <Link href={`/social-studies/p6/${topic.id}`} className="back">← {topic.title}</Link>
