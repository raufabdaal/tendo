...
export function generateStaticParams() {
  return P6_SCIENCE_TOPICS.map((t) => ({ topic: t.id }));
}

export default async function P6SciencePracticePage({
  params,
}: {
  ...
  return (
    <>
      <Link href={`/science/p6/${topic.id}`} className="back">← {topic.title}</Link>
